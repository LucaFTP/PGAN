...
class PGAN(Model):
    ...
    def init_generator(self):
        ...
        x = WeightScalingConv(x, filters = FILTERS[0], kernel_size=(3,3), gain=np.sqrt(2), activate='LeakyReLU', use_pixelnorm=False)
        self.conv_counter += 1 
        x = WeightScalingConv(x, filters = FILTERS[0], kernel_size=(2,2), gain=np.sqrt(2), activate='LeakyReLU', use_pixelnorm=True)
        self.conv_counter += 1
